...
function makeModel(projectRoot, app, sink, state) {
    ...
    const modelPath = path_1.join(modelsDirectory, `${state.modelName}.ts`);
...
function makeMiddleware(projectRoot, app, sink, state) {
     ...
     * Auth middleware
     */
    const authPath = path_1.join(middlewareDirectory, 'Auth.ts');
     ...
     */
    const silentAuthPath = path_1.join(middlewareDirectory, 'SilentAuth.ts');
...
function makeContract(projectRoot, app, sink, state) {
    ...
    const contractPath = path_1.join(contractsDirectory, 'auth.ts');
...
function makeConfig(projectRoot, app, sink, state) {
    ...
    const configPath = path_1.join(configDirectory, 'auth.ts');
