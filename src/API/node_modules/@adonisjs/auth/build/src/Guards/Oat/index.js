...
class OATGuard extends Base_1.BaseGuard {
     ...
     */
    generateHash(token) {
        return crypto_1.createHash('sha256').update(token).digest('hex');
    }
    /**
    ...
    generateTokenForPersistance(expiresIn) {
        const token = utils_1.randomString(this.tokenLength);
        return {
            token,
    ...
    parsePublicToken(token) {
         ...
         */
        const tokenId = utils_1.base64.urlDecode(parts[0], undefined, true);
        if (!tokenId) {
            throw AuthenticationException_1.AuthenticationException.invalidToken(this.name);
        }
        /**
    ...
    async login(user, options) {
         ...
         */
        const apiToken = new OpaqueToken_1.OpaqueToken(name, `${utils_1.base64.urlEncode(tokenId)}.${token.token}`, providerUser.user);
